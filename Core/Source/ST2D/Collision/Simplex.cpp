...
namespace ST
{
	...
	bool Simplex::containOrigin(const Simplex& simplex, bool strict)
	{
		...
		case 2:
			return Algorithm2D::checkPointOnSegment(simplex.vertices[0].result, simplex.vertices[1].result, { 0, 0 });
		case 3:
			return Algorithm2D::checkOriginInTriangle(simplex.vertices[0].result, simplex.vertices[1].result, simplex.vertices[2].result);
		default:
